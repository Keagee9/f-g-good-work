...
export default function ManageContentPage() {
  ...
  const handleGenerateUpdate = () => {
    try {
      // Validate that the text is valid JSON before proceeding
      const parsedServices = JSON.parse(servicesData);
      const parsedAddons = JSON.parse(addonsData);

      // Create the XML structure for the file changes
      const changesXml = `<changes>
  <description>Update services and add-ons data from admin content manager.</description>
  <change>
